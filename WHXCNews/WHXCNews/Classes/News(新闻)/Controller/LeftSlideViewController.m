...
- (void) handlePan: (UIPanGestureRecognizer *)rec{
    ...
    if (((self.mainVC.view.xj_x <= 0) && (_scalef <= 0)) || ((self.mainVC.view.xj_x >= (kScreenWidth - kMainPageDistance )) && (_scalef >= 0)))
    {
        //边界值管控
        _scalef = 0;
        needMoveWithTap = NO;
    ...
    {
        //超出范围，
        if (self.mainVC.view.xj_x < 0)
        {
            [self closeLeftView];
            _scalef = 0;
        }
        else if (self.mainVC.view.xj_x > (kScreenWidth - kMainPageDistance))
        {
            [self openLeftView];
            _scalef = 0;
        }
